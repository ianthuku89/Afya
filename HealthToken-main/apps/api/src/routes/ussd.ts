...
router.post('/', async (req, res) => {
    ...
    try {
        if (text === '') {
            // Main Menu
            response = `CON Welcome to AfyaToken SHIF, ${user.firstName || 'Member'}.\n`;
            ...
            if (level === 1) {
                ...
                response += `1. Opt-in\n`;
                response += `2. Opt-out\n`;
                response += `3. View Max Daily Ceiling`;
                ...
                await prisma.wallet.update({
                    where: { userId: user.id },
                    data: { autoDeductOptIn: false }
                });
                response = `END You have successfully opted out of M-PESA Auto-Deductions.`;
            } else if (level === 2 && textArray[1] === '3') {
                // View Max Daily Ceiling (Admin controlled)
                response = `END Your M-PESA Auto-Deduct daily ceiling is set to KES 300.\nTo protect users, this limit is system-controlled and cannot be manually modified.`;
            } else {
