...
coverageRouter.get('/qr', async (req, res, next) => {
    try {
        ...
        const [user, wallet, afyaScore] = await Promise.all([
            ...
            prisma.wallet.findUnique({ where: { userId }, select: { balanceAfya: true, coverageStatus: true } }),
        ...
        const isActive = wallet.coverageStatus === 'ACTIVE' && Number(wallet.balanceAfya) >= 10;
    ...
    try {
        ...
        const patient = await prisma.user.findUnique({
            ...
            select: {
                id: true,
                fullName: true,
                wallet: { select: { coverageStatus: true, balanceAfya: true, coverTypes: true } },
        ...
        const liveCoverageActive = patient.wallet.coverageStatus === 'ACTIVE' && Number(patient.wallet.balanceAfya) >= 10;
    ...
    try {
        ...
        const [wallet, afyaScore, recentVerifications] = await Promise.all([
            prisma.wallet.findUnique({
                where: { userId },
                select: { balanceAfya: true, coverageStatus: true, coverTypes: true, earnedMatchAfya: true },
            }),
