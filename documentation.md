...
*   **Auto-Deduct Sync:** The mobile app now records pending SHIF deductions immediately after STK push initiation using `/loyalty/auto-deduct/transaction-sync`. Deductions are capped at `KES 50` per day and any sync failures display a retry/failure alert directly to the user.
